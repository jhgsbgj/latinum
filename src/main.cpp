...
#if defined(NDEBUG)
# error "Latinum cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xbf59833bbc82f1854991da1d8d528bed63de9aaa7f76ec7411305ebb18ea1874");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Latinum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Latinum Signed Message:\n";
...
{
    // Latinum: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Mining reward halved every 256,000 blocks
    // Total supply: 25,600,000 (50 * 256000 * 2)
    nSubsidy >>= (nHeight / 256000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Latinum: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Latinum: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xe231b15f6f901c5bbf9eb8aaa423639ea931ba5fce8dac5146d973cefe432a8a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Dow 30 closes at 23,719.37 with a volume of 566,400,000 on 09/Apr/2020";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04dfe7ea186a383ce1fc02a19fe8b9043c820a9124fa1ab6033cdcee13984360b7d30c8aae2e7d46ecfa2f482220da2472bbc65da71faecc197b1e3e2405282729") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1586508755;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084955499;

        if (fTestNet)
        {
            block.nTime    = 1586508731;
            block.nNonce   = 385377636;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xbc1b6f2e3744b51ad7d694aa1a08df2dac39f34f9ca16c92771fd9ee058a7407"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf7, 0xc3, 0xb3, 0xd1 }; // Latinum: increase each by adding 2 to latinum's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("LatinumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LatinumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LatinumMiner(CWallet *pwallet)
{
    printf("LatinumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("latinum-miner");
    ...
    try { loop {
        ...
        printf("Running LatinumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LatinumMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LatinumMiner, pwallet));
}
